    ...
    if prob.item() > 0.75:
        ...
        for intent in intents['intents']:
            ...
            if tag == intent["tag"]:
                ...
                return reply
# print(TasksExecutor('send a message to me'))            
def MainTaskExecution(Query):
    Task = str(Query).lower()
    TaskNew = str(Query).lower()
    ReturnData = TasksExecutor(Task)
    
    if ReturnData is not None and "open" in ReturnData:
        Namen = str(TaskNew).replace("to","")
        Namen = str(Namen).replace("me","")
        Namen = str(Namen).replace("on","")
        Value = OpenExe(Namen)
        return Value
